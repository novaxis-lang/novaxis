...
class Executor {
	...
	/**
	 * @var ImportHandler The ImportHandler instance for handling import statements.
	 */
	...
	/**
	 * @var listCounter The listCounter instance for managing list counting operations.
	 */
	...
	*/
	...
	public function parameter(?string $previousLine, ?string $currentLine, ?string $oldcurrentline, ?string $nextLine, $firstline = false, $lastline = false, int $lineNumber) {
		...
		if ($this -> listCounterStarted['started']) {
			return $this -> executiveParts($currentLine, $oldcurrentline, [], $lineNumber, 'listCounter');
		}
		
		if ($tabHandling == 'backward' || $this -> wasInListCounter) {
			$this -> Path -> backward($this -> NestedTabsToBack + $this -> Tabs -> getDifferenceNumbers($this -> wasInListCounter ? $this -> listCounterLineTabs : $this -> Tabs -> getTabCountInLine($previousLine), $this -> Tabs -> getTabCountInLine($currentLine)));

			if ($this -> NestedTabsToBack) {
				$this -> NestedTabsToBack = null;
			}
			
			if ($this -> wasInListCounter) {
